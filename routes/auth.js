...
router.get('/login', (req, res) => {
    res.render('auth/login', { errorMessages: [], email: '', password: '' });
});

//Route To Make User LogIn 
router.post('/login',
    body('email').not().isEmpty().withMessage('Email Is Required').isEmail().withMessage('Please Enter Valid Email Address'),
        ...
        try {
            ...
            if (!errors.isEmpty()) {
                ...
                for (error of errors.array())
                    errs.push(error.msg)
                return res.render('auth/login', { errorMessages: errs, email: req.body.email, password: req.body.password });
            }

            let user = await UserModel.findOne({ email: req.body.email });
            console.log(user);
            if (!user)
                res.render('auth/login', { errorMessages: ['User Does Not Exist'], email: req.body.email, password: req.body.password });

            else if (!user.isVerified)
                res.render('auth/login', { errorMessages: ['Account Is Not Verified, Please Verify'], email: req.body.email, password: req.body.password });
            else {
                ...
                if (isPasswordMatches) {
                    ...
                    res.render('auth/login', { errorMessages: ['Invalid Credentials'], email: req.body.email, password: req.body.password });
                }
            }
        } catch (err) {
...
router.get('/register', (req, res) => {
    res.render('auth/register', {errorMessages: [], userData: {}});
});
    ...
    async (req, res) => {
        try {
            ...
            if (!errors.isEmpty()) {
                ...
                for (error of errors.array())
                    errs.push(error.msg)
                return res.render('auth/register', { errorMessages: errs, userData: req.body});
            }
            if(req.body.password != req.body.confirmPassword)
            return res.render('auth/register', { errorMessages: ["Password Does Not Match"], userData: req.body});
            //Using Async/Await
